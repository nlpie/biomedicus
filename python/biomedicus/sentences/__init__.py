...
from biomedicus.sentences import DeepMapper, vocabulary, Vocabulary
from biomedicus.sentences.models import deep_hparams_parser, BiLSTMSentenceModel, DeepMapper, \
    ensemble_hparams_parser, MultiSentenceModel, SavedModel
from biomedicus.sentences.processor import SentenceProcessor
from biomedicus.sentences.training import SentenceTraining, training_parser
...
def run_processor(args: Namespace, additional_args: List[str]):
    processor = create_processor(args, additional_args)
    nlpnewt.run_processor(processor, args)


def create_processor(args, additional_args):
    vocab = create_vocabulary(args)
    model, mapper = create_model(vocab, args, additional_args)
