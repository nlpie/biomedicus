...
def annotator_class(annotator_themes, annotator):
    try:
        annotator_result = any(annotator_themes.annotator_themes[annotator].values())
        return 1 if annotator_result else 0
    except KeyError:
        return 0


def process_for_agreement(input_dir, annotators_file):
    annotators = Path(annotators_file).read_text().splitlines()
    deserializer = JsonSerializer
    files = list(Path(input_dir).glob('*.json'))
    files.sort()

    with Path('any_theme.csv').open('w') as out:
        ...
        for f in tqdm(files):
            ...
            for annotator_themes in doc.labels['palliative_themes']:
                ...
                line = (sent_id,) + tuple(annotator_class(annotator_themes, annotator)
