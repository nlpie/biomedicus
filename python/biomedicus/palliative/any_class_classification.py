...
def main():
    ...
    def preprocess_function(examples):
        ...
        result["label"] = [(1 if l == 'theme' else 0) for l in examples["label"]]
    ...
    total = sum(class_sums)
    pos_weight = class_sums[0] / class_sums[1]
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        preds = [1 if logit >= 0 else 0 for logit in preds.reshape([-1])]
        if data_args.task_name is not None:
            ...
            precision, recall, f_score, _ = precision_recall_fscore_support(p.label_ids, preds, average='binary')
    ...
    logger.info(f"Using pos weight: {pos_weight}")
    trainer.pos_weight = torch.tensor([pos_weight], device="cuda:0")
                ...
                with open(output_predict_file, "w") as writer:
                    ...
                    for index, item in enumerate(predictions):
                        if is_regression:
                            ...
                            item = 'theme' if item == 1 else 'no_theme'
